-- Final SQL Script to update ThatInterpretingAgency database to relational schema
-- This script handles data migration before adding foreign key constraints
...
-- Step 2: Create temporary UserProfiles entries for existing data
PRINT 'Step 2: Creating temporary UserProfiles entries for existing data...';

-- Get all unique UserIds from existing tables
DECLARE @ExistingUserIds TABLE (UserId uniqueidentifier);

INSERT INTO @ExistingUserIds (UserId)
SELECT DISTINCT UserId FROM AgencyStaff WHERE UserId IS NOT NULL
UNION
SELECT DISTINCT UserId FROM Clients WHERE UserId IS NOT NULL
UNION
SELECT DISTINCT UserId FROM Interpreters WHERE UserId IS NOT NULL;

-- Create UserProfiles entries for existing UserIds
INSERT INTO UserProfiles (UserId, FirstName, LastName, CreatedAt, UpdatedAt)
SELECT 
    CAST(uid.UserId AS nvarchar(450)) AS UserId,
    'Temporary' AS FirstName,
    'User' AS LastName,
    GETDATE() AS CreatedAt,
    GETDATE() AS UpdatedAt
FROM @ExistingUserIds uid
WHERE NOT EXISTS (
    SELECT 1 FROM UserProfiles up WHERE up.UserId = CAST(uid.UserId AS nvarchar(450))
);

PRINT 'Created temporary UserProfiles entries for existing data';

GO

-- Step 3: Change column types from uniqueidentifier to nvarchar(450)
PRINT 'Step 3: Changing column types...';

-- First, update the data to convert uniqueidentifier to string
UPDATE AgencyStaff SET UserId = CAST(UserId AS nvarchar(450)) WHERE UserId IS NOT NULL;
UPDATE Clients SET UserId = CAST(UserId AS nvarchar(450)) WHERE UserId IS NOT NULL;
UPDATE Interpreters SET UserId = CAST(UserId AS nvarchar(450)) WHERE UserId IS NOT NULL;
UPDATE Appointments SET InterpreterId = CAST(InterpreterId AS nvarchar(450)) WHERE InterpreterId IS NOT NULL;
UPDATE Appointments SET ClientId = CAST(ClientId AS nvarchar(450)) WHERE ClientId IS NOT NULL;
UPDATE Invoices SET ClientId = CAST(ClientId AS nvarchar(450)) WHERE ClientId IS NOT NULL;

PRINT 'Updated data to convert uniqueidentifier to string';
...
PRINT 'Step 4: Updating table structure...';
...
-- Step 6: Add foreign key constraints to UserProfiles table
...
-- Step 7: Create migration history table if it doesn't exist
PRINT 'Step 7: Updating migration history...';
...
-- Show sample data from UserProfiles
SELECT TOP 10 * FROM UserProfiles;
...
PRINT 'Temporary UserProfiles entries were created for existing data.';
PRINT 'You can now update these entries with real user information.';
GO
